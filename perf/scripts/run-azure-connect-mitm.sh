...
K6_CONFIGMAP="${K6_CONFIGMAP:-aegis-k6-connect-mitm}"
K6_POD="${K6_POD:-aegis-k6-connect-mitm}"
...
TARGET_URL="${TARGET_URL:-$(tf_output nginx_https_url)}"
VUS="${VUS:-10}"
  ...
  while (( SECONDS < deadline )); do
    ...
    for ip in "${PROXY_IPS[@]}"; do
      if ! kubectl -n "${NAMESPACE}" exec "${pod}" -- sh -c \
        "curl -fsS http://${ip}:9090/metrics | grep -q 'aegis_policy_discovery_policies_active{provider=\"azure\",source=\"azure-policies\"} 3'"; then
        ready=0
        break
      fi
    done
...
create_k6_runner() {
  ...
  kubectl -n "${NAMESPACE}" create configmap "${K6_CONFIGMAP}" \
    --from-file=connect_mitm.js="${REPO_ROOT}/${K6_SCRIPT}" \
...
kubectl -n "${NAMESPACE}" exec "${K6_POD}" -- sh -c \
  "rm -rf '${K6_POD_RESULT_DIR}' && mkdir -p '${K6_POD_RESULT_DIR}' && k6 run /scripts/connect_mitm.js" \
